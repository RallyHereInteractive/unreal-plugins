...
void SRallyHereEditorWebRequestWidget::UpdateWebCallBox()
{
	...
	for (; requestsIterator; --requestsIterator) //$$ DLF - Fixed Debug crash caused by checking GetNode instead of using the bool operator
	{
		...
		TSharedPtr<SVerticalBox> MetadataDetails =
			...
			[
				...
				.AutoWidth()
				[
					SNew(STextBlock).Text(FText::FromString(*request->Metadata.Identifier.ToString(EGuidFormats::DigitsWithHyphensLower)))
				]
			]
			+SVerticalBox::Slot()
			.Padding(FMargin(2.0f))
