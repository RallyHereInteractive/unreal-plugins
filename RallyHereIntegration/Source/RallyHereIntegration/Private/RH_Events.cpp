...
namespace RHStandardEvents
{
	...
	TSharedRef<FJsonValue> FPlatformPurchaseReceiptEvent::FStoreData::ToJsonValue() const
	{
		auto JsonData = MakeShared<FJsonObject>();
	...
	TSharedRef<FJsonValue> FPlatformPurchaseReceiptEvent::FReceiptOfferData::ToJsonValue() const
	{
		auto JsonData = MakeShared<FJsonObject>();
		...
		if (StoreData.IsSet())
		{
			JsonData->SetField(TEXT("store_data"), StoreData.GetValue().ToJsonValue());
		}

		return MakeShared<FJsonValueObject>(JsonData);
	}

	TSharedRef<FJsonValue> FPlatformPurchaseReceiptEvent::FReceiptData::ToJsonValue() const
	{
		auto JsonData = MakeShared<FJsonObject>();
