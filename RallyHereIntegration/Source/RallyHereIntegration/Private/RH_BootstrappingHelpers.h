...
class FRH_ServerLoginUserHelper : public FRH_AsyncTaskHelper
{
public:
	FRH_ServerLoginUserHelper(FAuthContextPtr InAuthContext, FRH_ServerBootstrapLoginDelegate InDelegate)
	...
	virtual void Start(ERHAPI_GrantType GrantType, const FString& AccessToken)
	{
		Started();
		...
		Request.LoginRequestV1.PortalAccessToken = AccessToken;
		...
		HttpRequest = LoginType::DoCall(RH_APIs::GetAPIs().GetAuth(), Request,
			LoginType::Delegate::CreateSP(this, &FRH_ServerLoginUserHelper::RHLoginComplete),
...
protected:
	...
	void RHLoginComplete(const LoginType::Response& Resp)
	{
	...
	{
		static FString Name(TEXT("FRH_ServerLoginUserHelper"));
		return Name;
	}
...
// standalone helper that deals with initial login + refreshing token for the NULL user case
class FRH_ServerLoginNullHelper : public FRH_AsyncTaskHelper
{
public:
	FRH_ServerLoginNullHelper(FAuthContextPtr InAuthContext, FRH_ServerBootstrapLoginDelegate InDelegate)
		: AuthContext(InAuthContext)
		, Delegate(InDelegate)
	{
	}
	...
	virtual void Start()
	{
		...
		UE_LOG(LogRallyHereIntegration, Verbose, TEXT("[%s] Creating RallyHere Null Login request"), *GetName());
		HttpRequest = LoginType::DoCall(RH_APIs::GetAPIs().GetAuth(), Request,
			LoginType::Delegate::CreateSP(this, &FRH_ServerLoginNullHelper::RHLoginComplete),
			GetDefault<URH_IntegrationSettings>()->AuthLoginPriority);
...
protected:
	...
	void RHLoginComplete(const LoginType::Response& Resp)
	{
		if (Resp.IsSuccessful())
		{
			AuthContext->ProcessLoginToken(Resp);
			Completed(true);
		}
		else
		{
	...
	virtual FString GetName() const override
	{
		static FString Name(TEXT("FRH_ServerLoginNullHelper"));
		return Name;
	}
	virtual void ExecuteCallback(bool bSuccess) const override
	{
