...
struct FRH_DiagnosticReportOptions
{
	...
	/** The maximum number of web requests to include */
	UPROPERTY()
	int32 MaxWebRequests;
	...
	/** When writing to cloud, use the following as the correlation id */
	UPROPERTY()
	FString CloudCorrelationId;
...
protected:
	...
	*/
	void WriteToCloud();
	/** 
	* @brief Callback for when the cloud write is complete
	*/
	void OnWriteToCloudComplete(bool bSuccess, const FRH_ErrorInfo& ErrorInfo);
