...
void FRHDTW_WebRequests::DoViewRequest(const FRH_WebRequest* WebRequest)
{
	...
	ImGuiDisplayCopyableValue(TEXT("Verb"), WebRequest->Verb);
...
void FRHDTW_WebRequests::DoViewResponse(const FRH_WebResponse* WebResponse, const FRH_WebRequest* WebRequest)
{
	...
	ImGui::Text("Code : %d (%s)", WebResponse->ResponseCode, TCHAR_TO_UTF8(*EHttpResponseCodes::GetDescription(static_cast<EHttpResponseCodes::Type>(WebResponse->ResponseCode)).ToString()));
