...
struct RALLYHEREAPI_API FRHAPI_PortalUseRuleset : public FRHAPI_Model
{
	...
	*/
	...
	FORCEINLINE TMap<FString, TArray<ERHAPI_Portal>>& GetRules() { return Rules_Optional; }
	/** @brief Gets the value of Rules_Optional, regardless of it having been set */
	FORCEINLINE const TMap<FString, TArray<ERHAPI_Portal>>& GetRules() const { return Rules_Optional; }
	/** @brief Gets the value of Rules_Optional, if it has been set, otherwise it returns DefaultValue */
	FORCEINLINE const TMap<FString, TArray<ERHAPI_Portal>>& GetRules(const TMap<FString, TArray<ERHAPI_Portal>>& DefaultValue) const { if (Rules_IsSet) return Rules_Optional; return DefaultValue; }
	/** @brief Fills OutValue with the value of Rules_Optional and returns true if it has been set, otherwise returns false */
	FORCEINLINE bool GetRules(TMap<FString, TArray<ERHAPI_Portal>>& OutValue) const { if (Rules_IsSet) OutValue = Rules_Optional; return Rules_IsSet; }
	/** @brief Returns a pointer to Rules_Optional, if it has been set, otherwise returns nullptr */
	FORCEINLINE TMap<FString, TArray<ERHAPI_Portal>>* GetRulesOrNull() { if (Rules_IsSet) return (&Rules_Optional); return nullptr; }
	/** @brief Returns a pointer to Rules_Optional, if it has been set, otherwise returns nullptr */
	FORCEINLINE const TMap<FString, TArray<ERHAPI_Portal>>* GetRulesOrNull() const { if (Rules_IsSet) return (&Rules_Optional); return nullptr; }
	/** @brief Sets the value of Rules_Optional and also sets Rules_IsSet to true */
	FORCEINLINE void SetRules(const TMap<FString, TArray<ERHAPI_Portal>>& NewValue) { Rules_Optional = NewValue; Rules_IsSet = true;  }
	/** @brief Sets the value of Rules_Optional and also sets Rules_IsSet to true using move semantics */
	FORCEINLINE void SetRules(TMap<FString, TArray<ERHAPI_Portal>>&& NewValue) { Rules_Optional = NewValue; Rules_IsSet = true;  }
