...
FHttpRequestPtr FMatchAPI::CreateMatchRewards(const FRequest_CreateMatchRewards& Request, const FDelegate_CreateMatchRewards& Delegate /*= FDelegate_CreateMatchRewards()*/, int32 Priority /*= DefaultRallyHereAPIPriority*/)
{
	if (!IsValid())
		return nullptr;
	...
	// bind response handler
	FHttpRequestCompleteDelegate ResponseDelegate;
	ResponseDelegate.BindSP(this, &FMatchAPI::OnCreateMatchRewardsResponse, Delegate, RequestData->Metadata, Request.GetAuthContext(), Priority);
	RequestData->SetDelegate(ResponseDelegate);
...
void FMatchAPI::OnCreateMatchRewardsResponse(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded, FDelegate_CreateMatchRewards Delegate, FRequestMetadata RequestMetadata, TSharedPtr<FAuthContext> AuthContextForRetry, int32 Priority)
{
	FHttpRequestCompleteDelegate ResponseDelegate;
	...
	if (AuthContextForRetry)
	{
		// An included auth context indicates we should auth-retry this request, we only want to do that at most once per call.
		// So, we set the callback to use a null context for the retry
		ResponseDelegate.BindSP(this, &FMatchAPI::OnCreateMatchRewardsResponse, Delegate, RequestMetadata, TSharedPtr<FAuthContext>(), Priority);
	}

	TSharedRef<FResponse_CreateMatchRewards> Response = MakeShared<FResponse_CreateMatchRewards>(RequestMetadata);
...
FRequest_CreateMatchRewards::FRequest_CreateMatchRewards()
	: FRequest()
{
	RequestMetadata.SimplifiedPath = GetSimplifiedPath();
	RequestMetadata.SimplifiedPathWithVerb = GetSimplifiedPathWithVerb();
...
FName FRequest_CreateMatchRewards::GetSimplifiedPath() const
{
	static FName Path = FName(TEXT("/match/v1/match/{match_id}/reward"));
	return Path;
}

FName FRequest_CreateMatchRewards::GetSimplifiedPathWithVerb() const
{
	static FName PathWithVerb = FName(*FString::Printf(TEXT("POST %s"), *GetSimplifiedPath().ToString()));
	return PathWithVerb;
}

FString FRequest_CreateMatchRewards::ComputePath() const
{
	TMap<FString, FStringFormatArg> PathParams = { 
		{ TEXT("match_id"), ToStringFormatArg(MatchId) }
	};

	FString Path = FString::Format(TEXT("/match/v1/match/{match_id}/reward"), PathParams);

	return Path;
}

bool FRequest_CreateMatchRewards::SetupHttpRequest(const FHttpRequestRef& HttpRequest) const
{
	static const TArray<FString> Consumes = { TEXT("application/json") };
	//static const TArray<FString> Produces = { TEXT("application/json") };
	...
	if (!AuthContext && !PendingMetadataFlags.bDisableAuthRequirement)
	{
		UE_LOG(LogRallyHereAPI, Error, TEXT("FRequest_CreateMatchRewards - missing auth context"));
		return false;
	}
	if (AuthContext && !AuthContext->AddBearerToken(HttpRequest) && !PendingMetadataFlags.bDisableAuthRequirement)
	{
		UE_LOG(LogRallyHereAPI, Error, TEXT("FRequest_CreateMatchRewards - failed to add bearer token"));
		return false;
	}
	...
	if (Consumes.Num() == 0 || Consumes.Contains(TEXT("application/json"))) // Default to Json Body request
	{
		...
		WriteJsonValue(Writer, MatchRewardsBody);
		Writer->Close();
	...
	{
		UE_LOG(LogRallyHereAPI, Error, TEXT("FRequest_CreateMatchRewards - Body parameter (FRHAPI_MatchRewardsBody) was ignored, not supported in multipart form"));
	}
	else if (Consumes.Contains(TEXT("application/x-www-form-urlencoded")))
	{
		UE_LOG(LogRallyHereAPI, Error, TEXT("FRequest_CreateMatchRewards - Body parameter (FRHAPI_MatchRewardsBody) was ignored, not supported in urlencoded requests"));
	}
	else
	{
		UE_LOG(LogRallyHereAPI, Error, TEXT("FRequest_CreateMatchRewards - Request ContentType not supported (%s)"), *FString::Join(Consumes, TEXT(",")));
		return false;
	}
...
FString FResponse_CreateMatchRewards::GetHttpResponseCodeDescription(EHttpResponseCodes::Type InHttpResponseCode) const
{
	switch ((int)InHttpResponseCode)
	{
	case 201:
...
bool FResponse_CreateMatchRewards::ParseHeaders()
{
	if (!Super::ParseHeaders())
	{
		return false;
...
bool FResponse_CreateMatchRewards::TryGetContentFor201(FRHAPI_JsonObject& OutContent) const
{
	// if this is not the correct response code, fail quickly.
...
bool FResponse_CreateMatchRewards::TryGetContentFor400(FRHAPI_HzApiErrorModel& OutContent) const
{
	// if this is not the correct response code, fail quickly.
...
bool FResponse_CreateMatchRewards::TryGetContentFor403(FRHAPI_HzApiErrorModel& OutContent) const
{
	// if this is not the correct response code, fail quickly.
	if ((int)GetHttpResponseCode() != 403)
	{
...
bool FResponse_CreateMatchRewards::TryGetContentFor404(FRHAPI_HzApiErrorModel& OutContent) const
{
	// if this is not the correct response code, fail quickly.
	if ((int)GetHttpResponseCode() != 404)
	{
...
bool FResponse_CreateMatchRewards::TryGetContentFor422(FRHAPI_HTTPValidationError& OutContent) const
{
	// if this is not the correct response code, fail quickly.
	if ((int)GetHttpResponseCode() != 422)
	{
...
bool FResponse_CreateMatchRewards::FromJson(const TSharedPtr<FJsonValue>& JsonValue)
{
	bool bParsed = false;
	// for non default responses, parse into a temporary object to validate the response can be parsed properly
	switch ((int)GetHttpResponseCode())
...
FResponse_CreateMatchRewards::FResponse_CreateMatchRewards(FRequestMetadata InRequestMetadata)
	: Super(MoveTemp(InRequestMetadata))
{
}

FString Traits_CreateMatchRewards::Name = TEXT("CreateMatchRewards");

FHttpRequestPtr Traits_CreateMatchRewards::DoCall(TSharedRef<API> InAPI, const Request& InRequest, Delegate InDelegate, int32 InPriority)
{
	return InAPI->CreateMatchRewards(InRequest, InDelegate, InPriority);
}
...
FString FRequest_GetMatches::ComputePath() const
{
	...
	if(MinDurationSeconds.IsSet())
	{
		QueryParams.Add(FString(TEXT("min_duration_seconds=")) + ToUrlString(MinDurationSeconds.GetValue()));
	}
	if(Type.IsSet())
	{
...
FString FRequest_GetPlayerRecentlyPlayedWith::ComputePath() const
{
	...
	FString Path = FString::Format(TEXT("/match/v1/player/{player_uuid}/recently-played"), PathParams);
...
bool FResponse_GetPlayerRecentlyPlayedWith::FromJson(const TSharedPtr<FJsonValue>& JsonValue)
{
	...
	{  
		case 200:
			{
				// parse into the structured data format from the json object
				FRHAPI_RecentlyPlayedWithResponse Object;
				bParsed = TryGetJsonValue(JsonValue, Object);
