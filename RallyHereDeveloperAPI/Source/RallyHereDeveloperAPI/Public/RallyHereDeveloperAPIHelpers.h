...
#include "RallyHereDeveloperAPIHelpers.generated.h"

struct FRHAPI_DevJsonValue;
class IHttpRequest;

constexpr int32 DefaultRallyHereDeveloperAPIPriority = 1000000;

UENUM(BlueprintType)
enum class ERHAPI_DevJsonValueType : uint8
{
    None,
    Null,
    String,
...
struct RALLYHEREDEVELOPERAPI_API FRHAPI_DevJsonObject
{
    GENERATED_BODY()
public:
    FRHAPI_DevJsonValue TryGetValue(const FString& FieldName) const;

    bool HasField(const FString& FieldName) const;
	void SetField(const FString& FieldName, const FRHAPI_DevJsonValue& Value) const;
	...
	TArray<FRHAPI_DevJsonValue> GetArrayField(const FString& FieldName) const;
	bool TryGetArrayField(const FString& FieldName, TArray<FRHAPI_DevJsonValue>& OutArray) const;
	void SetArrayField(const FString& FieldName, const TArray<FRHAPI_DevJsonValue>& Array) const;

    FRHAPI_DevJsonObject GetObjectField(const FString& FieldName) const;
	bool TryGetObjectField(const FString& FieldName, FRHAPI_DevJsonObject& OutObject) const;
	void SetObjectField(const FString& FieldName, const FRHAPI_DevJsonObject& JsonObject) const;

    void SetObject(const TSharedPtr<FJsonObject> NewObj) { Obj = NewObj; }
    TSharedPtr<FJsonObject> GetObject() const { return Obj; }

    static FRHAPI_DevJsonObject CreateFromUnrealObject(TSharedPtr<FJsonObject> NewObj);
...
USTRUCT(BlueprintType)
struct RALLYHEREDEVELOPERAPI_API FRHAPI_DevJsonValue
{
    GENERATED_BODY()
public:
    ...
    bool AsBool() const;
    bool IsNull() const;
    TArray<FRHAPI_DevJsonValue> AsArray() const;
    FRHAPI_DevJsonObject AsObject() const;

    ERHAPI_DevJsonValueType GetType() const;
	...
	bool TryGetArray(TArray<FRHAPI_DevJsonValue>& OutArray) const;
	bool TryGetObject(FRHAPI_DevJsonObject& OutObject) const;

    void SetValue(const TSharedPtr<FJsonValue> NewValue) { Value = NewValue; }
    TSharedPtr<FJsonValue> GetValue() const { return Value; }

    static FRHAPI_DevJsonValue CreateFromUnrealValue(const TSharedPtr<FJsonValue> NewValue);

    bool CompareEqual(const FRHAPI_DevJsonValue& Other) const;
...
class RALLYHEREDEVELOPERAPI_API FHttpFileInput
{
public:
...
template<typename T, typename std::enable_if<!std::is_base_of<FRHAPI_DevModel, T>::value, int>::type = 0>
...
inline FString ToString(const FRHAPI_DevModel& Value)
{
	FString String;
...
inline void WriteJsonValue(TSharedRef<TJsonWriter<>>& Writer, const FRHAPI_DevJsonValue& Value)
{
...
inline void WriteJsonValue(TSharedRef<TJsonWriter<>>& Writer, const FRHAPI_DevJsonObject& Value)
{
...
inline void WriteJsonValue(TSharedRef<TJsonWriter<>>& Writer, const FRHAPI_DevModel& Value)
{
	Value.WriteJson(Writer);
}

template<typename T, typename std::enable_if<!TIsVariant<T>::Value && !std::is_base_of<FRHAPI_DevModel, T>::value, int>::type = 0>
...
inline bool TryGetJsonValue(const TSharedPtr<FJsonValue>& JsonValue, FRHAPI_DevJsonValue& Value)
{
	Value.SetValue(JsonValue);
	return true;
}

inline bool TryGetJsonValue(const TSharedPtr<FJsonValue>& JsonValue, FRHAPI_DevJsonObject& Value)
{
...
RALLYHEREDEVELOPERAPI_API bool ParseDateTime(const FString& DateTimeString, FDateTime& OutDateTime);
...
inline bool TryGetJsonValue(const TSharedPtr<FJsonValue>& JsonValue, FRHAPI_DevModel& Value)
{
